...
- [x] 남은 투입금액을 반환한다
  - [x] 최소개수의 동전으로 반환한다.
  - [x] 잔돈을 반환할 수 없으면 반환할 수 있는 금액만 반환한다.
  - [x] 잔돈을 반환하면 그만큼 줄어든다
...
- [x] 품절인지 알 수 있다
- [x] 판매할 수 있다
- [x] 일정 금액으로 살 수 있는지 알 수 있다.
- [x] 가격은 100원 이상이다
- [x] 가격은 10원 단위로 나누어 떨어진다

## 상품들
- [x] 상품명, 가격, 수량으로 상품을 추가한다.
- [x] 판매가 가능한지 알 수 있다
...
## 오류 핸들링
- [x] 잘못된 입력은 `IllegalArgumentException`를 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 해당 부분부터 다시 입력

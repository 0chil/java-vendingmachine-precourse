...
# 기능 목록
- [x] 보유 금액으로 동전들을 무작위로 생성한다
- [ ] 금액을 최소개수의 동전으로 반환한다
  - 지폐로 반환하지 않는다.
- [ ] 상품명, 가격, 수량으로 상품을 추가한다.
  - 재고는 음수가 될수 없다
  - 상품명은 공백일 수 없다
- [ ] 가격은 100원 이상이다
- [ ] 가격은 10원 단위로 나누어 떨어진다.
- [ ] 금액을 투입할 수 있다.
- [ ] 판매가 불가하면 즉시 반환한다.
  - 남은 금액으로 살 수 있는 제품이 없는 경우
  - 재고가 모두 소진된 경우
- [ ] 잘못된 입력은 `IllegalArgumentException`를 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 해당 부분부터 다시 입력
